const { getOptimizedSubset } = require("../7");

describe("Problem 7", () => {
  test("check total value and weight of all the items that have selected to be placed in the container", async () => {
    const values = [
    ...
    expect(getOptimizedSubset(values, weights, MAX_CAPACITY)).toEqual(
      expect.objectContaining({ totalWeight: 850, totalValue: 7534 })
    );
  });
});
